...
const userBubble = (
  <div className="bulle-user">
    <img
      className="avatar-bulles"
...
export const AnimateBulles = () => {
  ...
  useEffect(() => {
    if (userBubbles.length > 0) {
      const userBubblesHTML = Array.from(
  ...
  useEffect(() => {
    if (dwavesBubbles.length > 0) {
      const dwavesBubblesHTML = Array.from(
        document.getElementsByClassName(
          "bulle"
        ) as HTMLCollectionOf<HTMLElement>
      );
  ...
  const styleBubble = (bubble: HTMLElement, i: number) => {
    ...
    bubble.style.height =
      Math.random() * (maxHeight - minHeight) + minHeight + "px";
