...
class MusicController implements IMusicController {
    createSingleMusic: RequestHandler = async (req, res) => {
        ...
        const coverCID = await pinataService.pinFileToIPFS(fs.createReadStream(pathUploadsFilesDirectory + cover.name), {type: 'cover'})
        ...
        const musicCID = await pinataService.pinFileToIPFS(fs.createReadStream(pathUploadsFilesDirectory + music.name), musicMetadata)
