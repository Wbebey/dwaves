...
async function uploadMusicNFT() {
  ...
  const nFTMusic = new ethers.Contract(NFTMusic.address, NFTMusic.abi, signer)

  try {
    let transaction = await nFTMusic.createToken(
      'https://gateway.pinata.cloud/ipfs/QmRGRx18NFnPumLDjZaEYj2XRpgdEb4hnKFt1QdbEf6anM'
    )
